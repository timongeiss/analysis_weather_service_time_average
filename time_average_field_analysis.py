...
def generate_time_average_radiation(days=4, samples_per_hour=6, scaling_factor=150):
    ...
    data = pd.DataFrame({
        "time": time_index_datetime,
        "radiation": radiation_noisy
    })
    ...
    data["cumulative_area"] = (data["radiation"] * data["time_diff"]).cumsum()                                      # accumulated area for each time step (Integral) 
    data["cumulative_mean"] = data["cumulative_area"] / ((data.index + 1) * time_diff_hours)                        # average value over the entire interval from the start to the current point in time
...
def recalculate_original_values(hourly_mean):
    ...
    for i in range(1, len(hourly_mean_with_original)):
        t1 = (hourly_mean_with_original.at[i-1, "hour"] - hourly_mean_with_original.at[0, "hour"]).total_seconds() / 3600
        t2 = (hourly_mean_with_original.at[i, "hour"] - hourly_mean_with_original.at[0, "hour"]).total_seconds() / 3600
